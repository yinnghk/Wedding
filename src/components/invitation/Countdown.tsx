...
function Calendar() {
  ...
  return (
    <div className="p-2">
      {/* 12월 아래 여백 축소: mt-4 -> mt-2 */}
      <p className="text-center font-sans-serif text-lg">12월</p>
      
      {/* 요일과 숫자 사이 여백 확대: pt-3 및 gap-y-2.5 추가 */}
      <div className="mt-2 pt-3 grid grid-cols-7 gap-y-0.5 text-center text-xs">
        {weekdays.map((w, i) => (
          <div
            key={w}
            className={`font-medium pb-2 ${
              i === 0 ? "text-red-400" : i === 6 ? "text-blue-400" : "text-foreground/60"
            }`}
          >
            {w}
          </div>
    ...
    <section className="bg-[#ffffff] py-20 text-center">
      <div className="invitation-container">
        <div className="text-center">
          ...
          <p className="mt-7 leading-relaxed text-sm text-primary/80">
            2026년 12월 12일 토요일 오후 4시
          </p>
          <p className="mt-1 leading-relaxed text-sm text-primary/80">
            로얄파크컨벤션 1F 파크홀
          </p>
        </div>

        {/* 상단 구분선 py-8 -> py-2로 변경하여 아래 하단 구분선(py-2)과 동일하게 맞춤 */}
        ...
        <div className="py-4 p-2.5">
          <div className="grid grid-cols-4 gap-2 text-center">
            {[
              ...
              >
                <p className="sans-serif text-md tabular-nums text-foreground/80">
                  {String(c.v).padStart(2, "0")}
                </p>
                <p className="text-[9px] tracking-widest text-foreground/50 mt-0.5">
                  {c.l}
                </p>
              </div>
            ))}
